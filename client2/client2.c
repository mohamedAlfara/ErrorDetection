...
int main(void)
{
    ...
    printf("\n[Client2] Packet received:\n%s\n", buffer);

    char data[2048], method[64], incoming_control[2048];
    data[0] = method[0] = incoming_control[0] = '\0';
    ...
    printf("\nDATA            : %s\n", data);
    printf("METHOD          : %s\n", method);
    printf("Sent CONTROL    : %s\n", incoming_control);

    char computed[2048];
    generate_control_info(data, method, computed, sizeof(computed));

    printf("Computed CONTROL: %s\n", computed);

    if (strcmp(incoming_control, computed) == 0)
    {
        printf("\nStatus: DATA CORRECT\n");
    }
    else
